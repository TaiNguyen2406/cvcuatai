...
Public Class frmCNNhapKho
    ...
    Private Sub frmCNNhapKho_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        ...
        Else
            ...
            sql &= " SELECT NgayThang,PHIEUNHAPKHO.SoPhieu,Kho,IDKhachHang,SoPhieuDH,IDUser,TienTe,TyGia,TienTruocThue DonTe, TienTruocThue*TyGia TienTruocThue,TienThue,IDNguoiDat FROM PHIEUNHAPKHO "
            ...
            sql &= " SELECT PHIEUDATHANG.SoPhieu,PHIEUDATHANG.TienTruocThue*PHIEUDATHANG.TyGia TienTruocThue,PHIEUDATHANG.TienThue*PHIEUDATHANG.TyGia TienThue,PHIEUDATHANG.TienTe,PHIEUDATHANG.TyGia,KHACHHANG.ttcMa"
            ...
            If Not ds Is Nothing Then
                ...
                tbDonTe.EditValue = ds.Tables(0).Rows(0)("DonTe")
    ...
    Public Sub LoadPhieuCG(ByVal IDKhachHang As Object)
        ...
        Dim tb As DataTable = ExecuteSQLDataTable("SELECT SoPhieu,TienTruocThue*TyGia TienTruocThue,TienThue*TyGia TienThue,TienTe,TyGia,NHANSU.Ten AS TakeCare,PHIEUDATHANG.IDTakeCare FROM PHIEUDATHANG LEFT JOIN NHANSU ON NHANSU.ID=PHIEUDATHANG.IDTakeCare WHERE IDKhachHang=@IDKH AND PHIEUDATHANG.PheDuyet=1 ORDER BY SoPhieu DESC")
        If Not tb Is Nothing Then
    ...
    Public Sub LoadThamChieu(ByVal SoPhieuCG As Object)
        ...
        sql &= " TENDONVITINH.Ten AS TenDVT,CanNhap AS SoLuong,DATHANG.SoLuong AS SLDat,case when (select LoaiDH from PHIEUDATHANG where (DATHANG.SoPhieuPhu is  null and PHIEUDATHANG.Sophieu=DATHANG.Sophieu ) or (DATHANG.SoPhieuPhu is not null and PHIEUDATHANG.Sophieu=DATHANG.Sophieuphu))=1 then DATHANG.FOB else DATHANG.DonGia end DonGia,DATHANG.MucThue,DATHANG.NhapThue,"
        ...
        sql &= " WHERE (DATHANG.SoPhieuPhu is null  and DATHANG.Sophieu=@SP)"
        sql &= " or (DATHANG.SoPhieuPhu is not null and DATHANG.SoPhieuPhu=@SP )"
    ...
    Private Sub gdvPhieuCG_EditValueChanged(sender As System.Object, e As System.EventArgs) Handles gdvPhieuDH.EditValueChanged
        ...
        AddParameter("@PhieuTC0", gdvPhieuDH.EditValue)
        Dim _sql = "select LoaiDH from PHIEUDATHANG where SoPhieu=@PhieuTC0"
        Dim LoaiDH As Integer = ExecuteSQLScalar(_sql)
        Dim TyGiaTB As Double = 0
        If LoaiDH = 1 Then
            AddParameter("@PhieuTC0", gdvPhieuDH.EditValue)
            AddParameter("@SoPhieuDH", gdvPhieuDH.EditValue)
            _sql = "select isnull(sum(TyGia*Sotien)/sum(SoTien),0)  from UNC where PhieuTC0=@PhieuTC0 or PhieuTC1 in(Select SoPhieu from PHIEUNHAPKHO where SoPhieuDH=@SoPhieuDH)"
            TyGiaTB = ExecuteSQLScalar(_sql)
            If TyGiaTB > 0 Then
                tbTyGia.EditValue = TyGiaTB
            End If
        End If
    ...
    Private Sub btCal_Click(sender As System.Object, e As System.EventArgs) Handles btCal.Click
        ...
        tbDonTe.EditValue = 0
        For i As Integer = 0 To gdvVTCT.RowCount - 1
            tbTienTruocThue.EditValue += gdvVTCT.GetRowCellValue(i, "ThanhTien") * tbTyGia.EditValue
            If gdvVTCT.GetRowCellValue(i, "NhapThue") Then
                tbTienThue.EditValue += Math.Round((gdvVTCT.GetRowCellValue(i, "ThanhTien") * tbTyGia.EditValue * gdvVTCT.GetRowCellValue(i, "MucThue")) / 100, 2)
            End If
            tbDonTe.EditValue += gdvVTCT.GetRowCellValue(i, "ThanhTien")
        Next
    ...
    Private Sub btGhi_Click(sender As System.Object, e As System.EventArgs) Handles btGhi.Click
        ...
        Try
            ...
            AddParameter("@Tientruocthue", tbDonTe.EditValue) ' tbTienTruocThue.EditValue)
    ...
    Public Shared Function CapNhatPhanBoTamUng(SoDatHang As String) As String
        ...
        Try
            ...
            sql &= " - "
            sql &= "(select isnull(sum(sotien*TyGia),0) from THU WHERE MucDich = 109 AND PhieuTC0 = @SoDH) "
            sql &= " - "
            sql &= "(select isnull(sum(sotien*TyGia),0) from THUNH WHERE MucDich = 109 AND PhieuTC0 = @SoDH) "
            AddParameter("@SoDH", SoDatHang)
            ...
            sql &= " - "
            sql &= "(select isnull(sum(sotien),0) from THU WHERE MucDich = 109 AND PhieuTC0 = @SoDH) "
            sql &= " - "
            sql &= "(select isnull(sum(sotien),0) from THUNH WHERE MucDich = 109 AND PhieuTC0 = @SoDH) "
            AddParameter("@SoDH", SoDatHang)
            Dim TienTamUngDonTe As Double = ExecuteSQLDataTable(sql).Rows(0)(0)
            '********************************************
            'Lấy mức phân bổ tối đa theo tiền tạm ứng
            Dim _tienPhanBoMax As Double = TienTamUng
            Dim _tienPhanBoMaxDonTe As Double = TienTamUngDonTe
            '********************************************

            'Lấy danh sách xuất kho tương ứng với chào giá này
            AddParameter("@SoDH", SoDatHang)
            Dim dtXuatKho As DataTable = ExecuteSQLDataTable("SELECT SoPhieu, ((Tientruocthue+Tienthue)*TyGia)ThanhTien, (Tientruocthue+Tienthue) ThanhTienDonTe FROM PHIEUNHAPKHO WHERE SophieuDH = @SoDH ORDER BY NgayThang ASC")
            ...
            For Each r As DataRow In dtXuatKho.Rows
                ...
                Dim tienPhanBoDonTe As Double = r("ThanhTienDonTe") * TyLePhanBo
                '**********************************************
                'Lấy mức phân bổ tối đa theo tiền tạm ứng
                If tienPhanBo > _tienPhanBoMax Then
                    tienPhanBo = _tienPhanBoMax
                    tienPhanBoDonTe = _tienPhanBoMaxDonTe
                End If
                _tienPhanBoMax = _tienPhanBoMax - tienPhanBo
                _tienPhanBoMaxDonTe = _tienPhanBoMaxDonTe - tienPhanBoDonTe
                '********************************************

                If TienTamUng <> 0 Then
                    SoDuTamUng = SoDuTamUng - tienPhanBo
                    AddParameter("@PhanBoTamUng", tienPhanBo)
                    AddParameter("@PhanBoTamUngDonTe", tienPhanBoDonTe)
                ...
                Else
                    ...
                    AddParameter("@PhanBoTamUngDonTe", 0)
            ...
            AddParameter("@DaTamUngDonTe", TienTamUngDonTe)
    ...
    Private Sub cbTienTe_EditValueChanged(sender As Object, e As EventArgs) Handles cbTienTe.EditValueChanged
        Dim edit As LookUpEdit = CType(sender, LookUpEdit)
        Dim dr As DataRowView = edit.GetSelectedDataRow
        tbTyGia.EditValue = dr("TyGia")
    End Sub

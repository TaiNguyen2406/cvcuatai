...
Public Class frmYeuCauDi_DatHang
    ...
    Public DatTon As Boolean = False
    Private Sub frmYeuCauDi_DatHang_Load(sender As Object, e As System.EventArgs) Handles Me.Load
        TrangThaiDH.isAddNew = True
        rcbTrangThaiDH.DataSource = TrangThaiDatHang()
        cbTrangThaiDH.Properties.DataSource = TrangThaiDatHang()
        ...
        'Tai
        lueHTTCT.Properties.DataSource = TAI.tableHinhThucCT()
        lueHTTCT.EditValue = 2
        If Not KiemTraQuyenSuDungKhongCanhBao("Menu", Me.Parent.Tag, DanhMucQuyen.Admin) And Not KiemTraQuyenSuDungKhongCanhBao("Menu", Me.Parent.Tag, DanhMucQuyen.TPKinhDoanh) And Not KiemTraQuyenSuDungKhongCanhBao("Menu", Me.Tag, DanhMucQuyen.KeToan) Then
            lueHTTCT.Enabled = False
        Else
            lueHTTCT.Enabled = True
        End If
        'Tai
        ...
        If Not KiemTraQuyenSuDungKhongCanhBao("Menu", Me.Tag, DanhMucQuyen.QuyenSua) Then
            btCNDuLieuChung.Visible = False
        End If

        colNguoiDuyet.VisibleIndex = 22
...
#Region "Các sự kiên liên quan tới các cb lọc dữ liệu"
    ...
    Public Sub loadKhachHang()
        Dim sql As String = " SELECT ID,ttcMa,Ten,IDTakeCare,IDHinhThucTT, IDHinhThucTT2 FROM KHACHHANG WHERE ttcKhachhang <> 1 "
        If Not KiemTraQuyenSuDungKhongCanhBao("Menu", Me.Parent.Tag, DanhMucQuyen.Admin) And Not KiemTraQuyenSuDungKhongCanhBao("Menu", Me.Parent.Tag, DanhMucQuyen.TPKinhDoanh) Then
            sql &= " AND (IDTakeCare=@IDNV OR IDTakeCare is null)"
    ...
    Public Sub loadTakeCare()
        Dim tb As DataTable = ExecuteSQLDataTable("SELECT ID,Ten,Email FROM NHANSU WHERE Noictac=74 ")
        If Not tb Is Nothing Then
    ...
    Private Sub loadCbHinhThucTT()
        ...
        End If
        'Tai
        tb = ExecuteSQLDataTable("SELECT ID,GiaiThich,Nhom FROM DM_HINH_THUC_TT where TrangThai=1 ORDER BY Nhom asc, GiaiThich asc")
        If Not tb Is Nothing Then
    ...
    End Sub
    'Tai
    Private Sub loadHinhThucCT()

        Dim tb As DataTable = TAI.tableHinhThucCT()
        If Not tb Is Nothing Then
            lueHTTCT.Properties.DataSource = tb
            If tb.Rows.Count > 0 Then
                lueHTTCT.EditValue = tb.Rows(0)("Id")
            End If
        Else
            ShowBaoLoi(LoiNgoaiLe)
        End If
...
#Region "Tổng hợp"
    Private Sub LoadDSTongHop()
        ...
        'Tai
        sql &= " ,SLYCTon=(select isnull(sum(SoLuong),0) from DAT_TON_CHI_TIET where IdVatTu=Vattu.ID and isDuyet=0)"
        'Tai
        ...
        With gdvTongHopCT
            ...
            For i As Integer = 0 To .RowCount - 1
                If DatTon = False Then
                    ...
                    End If
                Else
                    .SetRowCellValue(i, "LuongDat", .GetRowCellValue(i, "SLYCTon"))
                End If
...
#Region "Đặt hàng"
    ...
    Private Sub btSua_ItemClick(sender As System.Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles btSua.ItemClick, mSua.ItemClick
        ...
        If Not tb Is Nothing Then
            If tb.Rows(0)(0) = 1 Then
                If Not gdvCT.GetFocusedRowCellValue("LoaiDH") = 2 Then
                    If Not KiemTraQuyenSuDungKhongCanhBao("Menu", Me.Parent.Tag, DanhMucQuyen.Admin) Then
                        ...
                        '  btLuuDatHang.Enabled = False
                        '    Exit Sub
                    End If
                End If
            Else
                If Not gdvCT.GetFocusedRowCellValue("LoaiDH") = 2 And gdvCT.GetFocusedRowCellValue("IDTakecare") <> Convert.ToInt32(TaiKhoan) Then
                    If Not KiemTraQuyenSuDungKhongCanhBao("Menu", Me.Parent.Tag, DanhMucQuyen.TPKinhDoanh) Then
                        ShowCanhBao("Bạn không có quyền sửa đặt hàng của người khác !")
                        Exit Sub
                    End If
                End If
            ...
            cbTakeCareDatHang.ToolTip = gdvCT.GetFocusedRowCellValue("Email")
    ...
    Private Sub btLuuDatHang_Click(sender As System.Object, e As System.EventArgs) Handles btLuuDatHang.Click
        ...
        End If
        If lueHTTCT.EditValue Is Nothing Then
            ShowCanhBao("Chưa chọn hình thức chứng từ !")
            Exit Sub
        End If
        If cbLoaiDH.EditValue = 2 Then
            luuDHNhapKhau()
        ...
        Dim tb As DataTable = ExecuteSQLDataTable("SELECT PheDuyet FROM PHIEUDATHANG WHERE SoPhieu='" & tbSoPhieuDH.EditValue & "'")
        If Not tb Is Nothing Then
            If tb.Rows.Count > 0 Then
                If tb.Rows(0)(0) = 1 Then
                    If Not gdvCT.GetFocusedRowCellValue("LoaiDH") = 2 Then
                    ...
                    End If
                Else
                    If Not gdvCT.GetFocusedRowCellValue("LoaiDH") = 2 And cbTakeCareDatHang.EditValue <> Convert.ToInt32(TaiKhoan) Then
                        If Not KiemTraQuyenSuDungKhongCanhBao("Menu", Me.Parent.Tag, DanhMucQuyen.TPKinhDoanh) Then
                            ShowCanhBao("Bạn không có quyền sửa đặt hàng của người khác !")
                            Exit Sub
                        End If
        ...
        Try
            ...
            AddParameter("@IDHinhThucCT", lueHTTCT.EditValue)
            'Tai
            ...
            For i As Integer = 0 To gdvDatHangCT.RowCount - 1
                ...
                If Not IsDBNull(gdvDatHangCT.GetRowCellValue(i, "IDDatHang")) Or Not gdvDatHangCT.GetRowCellValue(i, "IDDatHang") Is Nothing Then
                    If cbTrangThaiDH.EditValue = 1 Then
            ...
            '************* PHAN BO TAM UNG ************************'
            frmCNNhapKho.CapNhatPhanBoTamUng(tbSoPhieuDH.EditValue)
        ...
        If cbTrangThaiDH.EditValue = 1 Then
            ExecuteSQLNonQuery("Update PHIEUDATHANG set Hoanthanh = 1 where Sophieu = '" & tbSoPhieuDH.EditValue & "'")
        End If
    ...
    Private Sub btXacNhanDatHang_Click(sender As System.Object, e As System.EventArgs) Handles btXacNhanDatHang.Click
        ...
        If cbTrangThaiDH.EditValue <> 1 Then
            Try
                ...
                AddParameterWhere("@TT", cbTrangThaiDH.EditValue)
                If cbTrangThaiDH.EditValue = 2 Then
                    AddParameterWhere("@TG", Now)
                Else
                    AddParameterWhere("@TG", DBNull.Value)
                End If

                sql = "Update PHIEUDATHANG Set Pheduyet = @TT , TGDeNghi=@TG,TGDuyet=null Where Sophieu = '" & tbSoPhieuDH.EditValue & "'"
                If ExecuteSQLNonQuery(sql) Is Nothing Then Throw New Exception("Không thực hiện được thao tác đề nghị duyệt: " & LoiNgoaiLe)

                ' tbNgayDeNghi.EditValue = Now
        ...
        Try
            ...
            sql = "Update PHIEUDATHANG Set Pheduyet = @TT , TGDuyet=@TG, IDNguoiduyet = " & TaiKhoan & " Where Sophieu = '" & tbSoPhieuDH.EditValue & "'"
            ...
            ComitTransaction()
            Dim str As String = "- Số ĐH: " & tbSoPhieuDH.EditValue & " <Br/>"
            str &= "- NCC: " & gdvNhaCCDatHang.Text & " <Br/>"
            str &= "- Người duyệt: " & NguoiDung & " <Br/>"
            str &= "- Thời gian: " & tg.ToString("dd/MM/yyyy HH:mm")

            Utils.Email.Send(cbTakeCareDatHang.ToolTip.ToString, "Đặt hàng: " & tbSoPhieuDH.EditValue & " - NCC:" & gdvNhaCCDatHang.Text & " đã được duyệt", str)
            ' chkDuyetDatHang.Checked = _PheDuyet
            Dim index As Object = gdvCT.FocusedRowHandle
            loadDSTongHopVatTuDatHang()
            gdvCT.FocusedRowHandle = index
...
#Region "Tổng hợp đặt hàng"
    Public Sub loadDSTongHopVatTuDatHang()
        ...
        sql &= "	PHIEUDATHANG.TienTruocThue,PHIEUDATHANG.TienThue,PHIEUDATHANG.TyGia,NHANSU_2.Email,PHIEUDATHANG.IDTakecare,"
        ...
        sql &= "	PHIEUDATHANG.FileDinhKem,PHIEUDATHANG.PheDuyet,PHIEUDATHANG.LoaiDH,tblTuDien.NoiDung AS TenLoaiDH, tblHinhThucTTKH.HinhThucTT_VIE AS HinhThucTT, DM_HINH_THUC_TT.GiaiThich as HinhThucTT2 ,'' AS CBTT, (CASE WHEN tb.SoTien is null then 0 ELSE (CASE WHEN tb.SoTien <> (PHIEUDATHANG.TienTruocThue + PHIEUDATHANG.TienThue) THEN 2 ELSE 1 END) END) AS TTTT, "
        sql &= "    (CASE PHIEUDATHANG.PheDuyet WHEN 0 THEN N'Soạn thảo' WHEN 1 THEN N'Đã duyệt' ELSE N'Chờ duyệt' END) as TrangThai,PHIEUDATHANG.NoiDungDeNghi,PHIEUDATHANG.NoiDungPhanHoi,PHIEUDATHANG.TGDeNghi,PHIEUDATHANG.TGDuyet"
        sql &= " , REPLACE(STR((DATEDIFF(minute,PHIEUDATHANG.TGDeNghi,PHIEUDATHANG.TGDuyet) / 60), case when len(cast((DATEDIFF(minute,PHIEUDATHANG.TGDeNghi,PHIEUDATHANG.TGDuyet) / 60) as varchar)) <= 1 then 2 else len(cast((DATEDIFF(minute,PHIEUDATHANG.TGDeNghi,PHIEUDATHANG.TGDuyet) / 60) as varchar)) end), SPACE(1), '0') + ':' + convert(varchar(2),PARSENAME(cast(DATEDIFF(minute,PHIEUDATHANG.TGDeNghi,PHIEUDATHANG.TGDuyet) as float) / cast(60 as float),1)) TGXL "
        sql &= ", PHIEUDATHANG.Hoanthanh "
        ...
        sql &= "        LEFT JOIN DM_HINH_THUC_TT ON DM_HINH_THUC_TT.ID = PHIEUDATHANG.IDHinhThucTT2"
    ...
    Public Sub loadDSVatTuDatHang(ByVal SoPhieu As Object, Optional ByVal LoaiDH As Integer = 0)
        ...
        Dim sql As String = ""
        sql &= " SELECT NgayDat,NgayNhan,IDKhachHang,SoPhieu,DieuKienKhac,TyGia,IDUser,TienTe, IDNgd,IDTakeCare,TGDeNghi,"
        sql &= " 		TienTruocThue,TienThue,PheDuyet,IDNguoiDuyet,FileDinhKem,IDHinhThucTT, LoaiDH,SoPhieuO"
        'Tai
        sql &= " ,IDHinhThucTT2, isnull(IDHinhThucCT,2) IDHinhThucCT"
        'Tai
        ...
        If Not ds Is Nothing Then
            ...
            If _exit = False Then
                If ds.Tables(0).Rows.Count > 0 Then
                    ...
                    cbTrangThaiDH.EditValue = ds.Tables(0).Rows(0)("PheDuyet")
                    ...
                    '  tbNgayDeNghi.EditValue = ds.Tables(0).Rows(0)("TGDeNghi")
                    cbHinhThucTT2.EditValue = ds.Tables(0).Rows(0)("IDHinhThucTT2")
                    lueHTTCT.EditValue = ds.Tables(0).Rows(0)("IDHinhThucCT")
                Else
                    ...
                    tbSoPhieuDH.Tag = ""
                    cbHinhThucTT2.EditValue = Nothing
                    lueHTTCT.EditValue = Nothing
                End If
            End If
    ...
    Private Sub btXem_ItemClick(sender As System.Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles btXem.ItemClick
        ...
        If cbTrangThai.EditValue = 2 Then
            colCBTT.VisibleIndex = 0
            colSoDH.VisibleIndex = 1
            colSoDHOmron.VisibleIndex = 2
            colMaKH.VisibleIndex = 3
            colTenKH.VisibleIndex = 4
            colTruocThue.VisibleIndex = 5
            colThue.VisibleIndex = 6
            GridColumn22.VisibleIndex = 7
            colNoiDungDN.VisibleIndex = 8
            colTakeCare.VisibleIndex = 9
            colTienTe.VisibleIndex = 10
            colNoiDungPH.VisibleIndex = 11
            colTGDeNghi.VisibleIndex = 12
            colTGDuyet.VisibleIndex = 13
            colNguoiDuyet.VisibleIndex = 14

            colPhieuNhap.VisibleIndex = 14
            colThoiGian.VisibleIndex = 15
            colNgayNhan.VisibleIndex = 16
            colNguoiGiaoDich.VisibleIndex = 17
            colNguoiLap.VisibleIndex = 18
            colTyGia.VisibleIndex = 19
            colLoaiDH.VisibleIndex = 20
            colTrangThai.VisibleIndex = 21
            colFile.VisibleIndex = 22
            colMaKH.Width = 75
            colTenKH.Width = 150
        Else
            colCBTT.VisibleIndex = 0
            colSoDH.VisibleIndex = 1
            colSoDHOmron.VisibleIndex = 2
            colPhieuNhap.VisibleIndex = 3
            colThoiGian.VisibleIndex = 4
            colMaKH.VisibleIndex = 5
            colTenKH.VisibleIndex = 6
            colNguoiGiaoDich.VisibleIndex = 7
            colTakeCare.VisibleIndex = 8
            colNgayNhan.VisibleIndex = 9
            colTruocThue.VisibleIndex = 10
            colThue.VisibleIndex = 11
            GridColumn22.VisibleIndex = 12
            colTienTe.VisibleIndex = 13
            colTyGia.VisibleIndex = 14
            colNguoiLap.VisibleIndex = 15
            colLoaiDH.VisibleIndex = 16
            colNoiDungDN.VisibleIndex = 17
            colNoiDungPH.VisibleIndex = 18
            colTGDeNghi.VisibleIndex = 19
            colTGDuyet.VisibleIndex = 20
            colNguoiDuyet.VisibleIndex = 21
    ...
    Private Sub gdvCT_FocusedRowChanged(sender As System.Object, e As DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs) Handles gdvCT.FocusedRowChanged
        If Not gdvCT.FocusedRowHandle < 0 Then
            loadDSVatTuDatHangChiTiet(gdvCT.GetFocusedRowCellValue("SoPhieu"), gdvCT.GetFocusedRowCellValue("LoaiDH"))
            If gdvCT.GetFocusedRowCellValue("PheDuyet") = 1 Then
                btDuyetDH.Caption = "Bỏ duyệt đặt hàng"
            Else
                btDuyetDH.Caption = "Duyệt đặt hàng"
            End If
        Else
...
#End Region
    ...
    Private Sub mDuyetDH_ItemClick(sender As System.Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles mDuyetDH.ItemClick, btDuyetDH.ItemClick
        ...
        If gdvCT.GetFocusedRowCellValue("PheDuyet") = 0 Then
            ShowCanhBao("Đặt hàng chưa được đề nghị duyệt !")
            Exit Sub
        End If
        ...
        Try
            ...
            If gdvCT.GetFocusedRowCellValue("PheDuyet") = 1 Then
                ...
                sql = "Update PHIEUDATHANG Set Pheduyet = 0 ,TGDeNghi=Null,TGDuyet=Null, IDNguoiduyet = " & TaiKhoan & " Where Sophieu = '" & gdvCT.GetFocusedRowCellValue("SoPhieu") & "'"
            ElseIf gdvCT.GetFocusedRowCellValue("PheDuyet") = 2 Then
                AddParameterWhere("@TG", tg)
                sql = "Update PHIEUDATHANG Set Pheduyet = 1 ,TGDuyet=@TG, IDNguoiduyet = " & TaiKhoan & " Where Sophieu = '" & gdvCT.GetFocusedRowCellValue("SoPhieu") & "'"
            End If
            ...
            With gdvChiTietDHCT
                Dim _RowCount = .RowCount - 1
                For i As Integer = 0 To _RowCount
                    If Not gdvCT.GetFocusedRowCellValue("PheDuyet") = 1 Then
                        sql = "Update DATHANG Set Cannhap = (Soluong - (select (isnull(sum(soluong),0)) from NHAPKHO where IDDathang = " & .GetRowCellValue(i, "IDDatHang") & ")) Where ID = " & .GetRowCellValue(i, "IDDatHang")
                    Else
                        sql = "Update DATHANG Set Cannhap = 0 Where ID = " & .GetRowCellValue(i, "IDDatHang")
                    End If
            ...
            ComitTransaction()
            If gdvCT.GetFocusedRowCellValue("PheDuyet") = 2 Then
                Dim str As String = "- Số ĐH: " & gdvCT.GetFocusedRowCellValue("SoPhieu") & " <Br/>"
                str &= "- NCC: " & gdvCT.GetFocusedRowCellValue("MaKH") & " <Br/>"
                str &= "- Người duyệt: " & NguoiDung & " <Br/>"
                str &= "- Thời gian: " & tg.ToString("dd/MM/yyyy HH:mm")
                ThemThongBaoChoNV("Đặt hàng: " & gdvCT.GetFocusedRowCellValue("SoPhieu") & " - NCC:" & gdvCT.GetFocusedRowCellValue("MaKH") & " đã được duyệt", gdvCT.GetFocusedRowCellValue("IDTakecare"))
                Utils.Email.Send(gdvCT.GetFocusedRowCellValue("Email"), "Đặt hàng: " & gdvCT.GetFocusedRowCellValue("SoPhieu") & " - NCC:" & gdvCT.GetFocusedRowCellValue("MaKH") & " đã được duyệt", str)
            ...
            Else
                ...
                ThemThongBaoChoNV("Đặt hàng: " & gdvCT.GetFocusedRowCellValue("SoPhieu") & " - NCC:" & gdvCT.GetFocusedRowCellValue("MaKH") & " đã bị hủy", gdvCT.GetFocusedRowCellValue("IDTakecare"))
                Utils.Email.Send(gdvCT.GetFocusedRowCellValue("Email"), "Đặt hàng: " & gdvCT.GetFocusedRowCellValue("SoPhieu") & " - NCC:" & gdvCT.GetFocusedRowCellValue("MaKH") & " đã bị hủy", str)
    ...
    Private Sub btCanXuat_Click(sender As System.Object, e As System.EventArgs) Handles btCanXuat.Click
        ...
        End If
        If listVTCX.Rows.Count = 0 Then
            ShowCanhBao("Hàng cần xuất số lượng chưa khớp !")
            Exit Sub
        End If
        ...
        sql &= " ,SLTon=((select isnull(SUM(Soluong),0) from V_NHAPKHO where IDVattu=Vattu.ID) -(select isnull(SUM(Soluong),0) from V_XUATKHO where IDVattu=Vattu.ID))"
        'Tai
        sql &= " ,SLYCTon=(select isnull(sum(SoLuong),0) from DAT_TON_CHI_TIET where IdVatTu=Vattu.ID and isDuyet=1)"
        'Tai
        sql &= " ,CanNhap=(select isnull(SUM(Cannhap),0) from DATHANG where IDVattu=Vattu.ID) "
        sql &= " ,Convert(float,0)LuongDat "
        sql &= " ,KHSD=ISNULL(tbKHSD.KHSD,0)"
...
#Region "Tài"
    ...
    End Sub
#End Region
    Private Sub rcbTrangThaiDH_ButtonClick(sender As System.Object, e As DevExpress.XtraEditors.Controls.ButtonPressedEventArgs) Handles rcbTrangThaiDH.ButtonClick
        If e.Button.Index = 1 Then
            cbTrangThai.EditValue = Nothing
        End If
    End Sub

    Private Sub cbTakeCareDatHang_EditValueChanged(sender As System.Object, e As System.EventArgs) Handles cbTakeCareDatHang.EditValueChanged
        '  On Error Resume Next
        Dim dr As DataRowView = CType(sender, LookUpEdit).GetSelectedDataRow
        If dr IsNot Nothing Then
            If Not IsDBNull(dr("Email")) Then
                cbTakeCareDatHang.ToolTip = dr("Email")
            End If

        End If
    End Sub

    Private Sub mPhanHoiDonHang_ItemClick(sender As System.Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles mPhanHoiDonHang.ItemClick
        If Not KiemTraQuyenSuDungKhongCanhBao("Menu", Me.Parent.Tag, DanhMucQuyen.Admin) Then
            ShowBaoLoi("Bạn không có quyền thực hiện thao tác này !")
            Exit Sub
        End If
        If gdvCT.GetFocusedRowCellValue("PheDuyet") = 1 Then
            ShowCanhBao("Đặt hàng đã duyệt !")
            Exit Sub
        End If

        If gdvCT.GetFocusedRowCellValue("PheDuyet") = 0 Then
            ShowCanhBao("Đặt hàng đang soạn thảo !")
            Exit Sub
        End If
        ...
        If gdvCT.GetFocusedRowCellValue("PheDuyet") = 2 Then
            Dim f As New frmDatHang_PhanHoi
            f.Text = "Phản hồi về đơn hàng"
            f._DeNghi = False
            f._EmailPhuTrach = gdvCT.GetFocusedRowCellValue("Email")
            f._NCC = gdvCT.GetFocusedRowCellValue("MaKH")
            f._PhuTrach = gdvCT.GetFocusedRowCellValue("IDTakecare")
            f.lbThongTin.Text = "Thông tin phản hồi đơn hàng"
    ...
    Private Sub btXacNhan_Click(sender As System.Object, e As System.EventArgs)
        ...
        ' Utils.Email.Send(gdvCT.GetFocusedRowCellValue("Email"), "Cần điều chỉnh lại đặt hàng: " & gdvCT.GetFocusedRowCellValue("SoPhieu") & " - NCC: " & gdvCT.GetFocusedRowCellValue("MaKH"), tbNoiDungPH.EditValue)
        'gPhanHoi.Visible = False
    End Sub



    Private Sub btDeNghiDuyet_Click(sender As System.Object, e As System.EventArgs) Handles btDeNghiDuyet.Click
        If cbTrangThaiDH.EditValue = 0 Then
            If tbSoPhieuDH.EditValue Is Nothing Then
                Exit Sub
            End If
            Dim f As New frmDatHang_PhanHoi
            f.Text = "Đề nghị duyệt đặt hàng"
            f._EmailPhuTrach = cbTakeCareDatHang.ToolTip
            f._PhuTrach = cbTakeCareDatHang.EditValue
            f._NCC = gdvNhaCCDatHang.Text
            f.lbThongTin.Text = "Thông tin đề nghị duyệt đặt hàng"
            f._SoPhieu = tbSoPhieuDH.EditValue
            f.ShowDialog()
        End If

    End Sub

    Private Sub cbTrangThaiDH_EditValueChanged(sender As System.Object, e As System.EventArgs) Handles cbTrangThaiDH.EditValueChanged
        If cbTrangThaiDH.EditValue = 0 Then
            btDeNghiDuyet.Visible = True
        Else
            btDeNghiDuyet.Visible = False
        End If
    End Sub

    Private Sub mDeNghiDuyetDonHang_ItemClick(sender As System.Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles mDeNghiDuyetDonHang.ItemClick
        If gdvCT.GetFocusedRowCellValue("PheDuyet") = 0 Then
            Dim f As New frmDatHang_PhanHoi
            f.Text = "Đề nghị duyệt đặt hàng"
            If gdvCT.GetFocusedRowCellValue("Email") IsNot Nothing Then
                f._EmailPhuTrach = gdvCT.GetFocusedRowCellValue("Email")
            End If
    ...
    Private Sub btCNDuLieuChung_Click(sender As System.Object, e As System.EventArgs) Handles btCNDuLieuChung.Click
        If IsDBNull(lueHTTCT.EditValue) Then
        ...
        If Not KiemTraQuyenSuDungKhongCanhBao("Menu", Me.Parent.Tag, DanhMucQuyen.Admin) Then
            ...
            End If
        End If
        If TrangThaiDH.isAddNew Then Exit Sub
    ...
    Private Function CheckHinhThucThanhToanMoiCoLoi()
        Try
            ...
            AddParameter("@IdKH", gdvNhaCCDatHang.EditValue)

    ...
    Private Sub btUpdateDiem2_ItemClick(sender As System.Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles btUpdateDiem2.ItemClick
        ...
        Try
            For i As Integer = 0 To gdvCT.RowCount - 1
                ...
                If doUpdate("tblTHChamCong", "Month=@Month AND IDNhanVien=@IDNhanVien") Is Nothing Then Throw New Exception(LoiNgoaiLe)
                sql = ""
                sql &= " IF EXISTS(SELECT * FROM NhanSu_LoiNhuan WHERE IDNhanVien=@IDNhanVien AND Thang=@Thang AND IDNhom=@Nhom)"
                sql &= " BEGIN"
                sql &= "    Update NhanSu_LoiNHuan"
                sql &= " 	SET SoTien=@SoTien WHERE IDNhanVien=@IDNhanVien AND Thang=@Thang AND IDNhom=@Nhom "
                sql &= " End"
                sql &= " Else"
                sql &= " BEGIN"
                sql &= " 	INSERT INTO NhanSu_LoiNhuan(IDNhanVien,Thang,SoTien,IDNhom,isLNTaoRa)"
                sql &= " 	VALUES(@IDNhanVien,@Thang,@SoTien,@Nhom,1)"
                sql &= " End"
                AddParameterWhere("@Thang", cbThang.EditValue.ToString & "/" & tbNam.EditValue.ToString)
                AddParameterWhere("@IDNhanVien", gdvCT.GetRowCellValue(i, "IDNhanVien"))
                AddParameterWhere("@SoTien", gdvCT.GetRowCellValue(i, "LoiNhuan"))
                If cbTieuChi.EditValue = "Xuất kho" Then
                    AddParameterWhere("@Nhom", 1)
                Else
                    AddParameterWhere("@Nhom", 2)
                End If
    ...
    Private Sub btXemCachMoi_ItemClick(sender As System.Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles btXemCachMoi.ItemClick
        ...
        If Not tb Is Nothing Then
            For i As Integer = 0 To tb.Rows.Count - 1
                ...
                If tb.Rows(i)("PhanTramMoi") <= tb.Rows(i)("DiemDo") Then
                    tb.Rows(i)("Mau") = "R"
                ElseIf tb.Rows(i)("PhanTramMoi") > tb.Rows(i)("DiemDo") And tb.Rows(i)("PhanTramMoi") <= tb.Rows(i)("DiemVang") Then
                    tb.Rows(i)("Mau") = "Y"
                ElseIf tb.Rows(i)("PhanTramMoi") > tb.Rows(i)("DiemVang") And tb.Rows(i)("PhanTramMoi") <= tb.Rows(i)("DiemXanh") Then
                    tb.Rows(i)("Mau") = "G"
                Else
                    tb.Rows(i)("Mau") = "S"
                End If

...
Public Class frmYeuCauDen
    ...
    Private Sub frmYeuCauDen_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        If Not KiemTraQuyenSuDungKhongCanhBao("Menu", Me.Parent.Tag, DanhMucQuyen.TPKinhDoanh) And Not KiemTraQuyenSuDungKhongCanhBao("Menu", Me.Parent.Tag, DanhMucQuyen.Admin) Then
            colSoDTNGD.Visible = False
            colSDTEmailGD.Visible = False
            cbNVKD.Enabled = False
        End If
...
#Region "Load DS dữ liệu vào các combobox và grid"
    ...
    Public Sub LoadDSYeuCau2(ByVal _SoPhieu As Object)
        ...
        sql &= " LEFT OUTER JOIN NHANSU AS NHANSU2 ON BANGYEUCAU.IDNgd=NHANSU2.ID "
        sql &= " LEFT OUTER JOIN NHANSU AS NGUOIXL ON BANGYEUCAU.IDNhanXL=NGUOIXL.ID "
        sql &= " WHERE BANGYEUCAU.SoPhieu=N'" & _SoPhieu & "'"
    ...
    Public Sub loadDSYCChiTiet(ByVal SoPhieu As Object)
        ...
        sql &= " )"
        sql &= " INSERT INTO @tb"
        sql &= " SELECT Convert(bit,0)Chon,ROW_NUMBER() OVER(ORDER BY YEUCAUDEN.ID ) AS STT, TENVATTU.ten AS TenVT, TENHANGSANXUAT.TEN AS TenHang, VATTU.Model, TENDONVITINH.TEN AS DVT, VATTU.Thongso, "
        ...
        sql &= "      VATTU.HangTon,NGUOINHANBAOGIA.Ten AS NguoiNhanBaoGia,YEUCAUDEN.IDBo,YEUCAUDEN.SLBo,tblGhepVatTu.SoLuong AS SLTrongBo"
        sql &= " FROM    BANGYEUCAU "
        sql &= "     LEFT JOIN KHACHHANG ON BANGYEUCAU.IDkhachhang = KHACHHANG.ID "
        sql &= "     RIGHT JOIN YEUCAUDEN ON BANGYEUCAU.Sophieu = YEUCAUDEN.Sophieu "
        sql &= "    LEFT JOIN tblGhepVatTu ON tblGhepVatTu.IDVatTu=YEUCAUDEN.IDBo AND tblGhepVatTu.IDVatTuPhu=YEUCAUDEN.IDVatTu"
        sql &= "     LEFT JOIN VATTU ON YEUCAUDEN.IDVattu = VATTU.ID "
        sql &= "     LEFT JOIN TENNHOM ON VATTU.IDTennhom = TENNHOM.ID "
        sql &= "     LEFT JOIN TENTAILIEU ON VATTU.IDTentailieu = TENTAILIEU.ID "
        ...
        sql &= " SELECT * FROM @tb ORDER BY STT"
    ...
    Public Sub loadCbTuDien()
        ...
        If Not dt Is Nothing Then
            ...
            Select Case Me.Tag
                Case "MUAHANG"
                    cbTrangThaiYC.EditValue = TrangThaiYeuCau.CanHoiGiaHang
                Case "KYTHUAT"
            ...
            If Me.Tag = "KINHDOANH" Then cbNVKD.EditValue = Convert.ToInt32(TaiKhoan)
            'rcbKH.DataSource = dt.Tables(4)
            'rbtFilterMaKH.DataSource = dt.Tables(4)
            'rcbTienTe.DataSource = dt.Tables(5)
            rcbTienTe.DataSource = dt.Tables(4)
        Else
            ShowBaoLoi(LoiNgoaiLe)
        End If
...
#Region "Các sự kiện của thanh công cụ và menu chính"
    ...
    Private Sub btNhanChuyenMa_ItemClick(sender As System.Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles mNhanChuyenMa.ItemClick
        ...
        Dim tb As DataTable = ExecuteSQLDataTable("SELECT IDChuyenma FROM YEUCAUDEN WHERE ID=@ID")
        If Not tb Is Nothing Then
            If tb.Rows(0)(0).ToString <> "" Then
                ShowCanhBao("Đã có người nhận chuyển mã cho yêu cầu này!")
                Exit Sub
            End If
        End If

        Dim tg As DateTime = GetServerTime()
        AddParameter("@IDChuyenma", Convert.ToInt32(TaiKhoan))
    ...
    Private Sub ChaoGia_ItemClick(ByVal sender As System.Object, ByVal e As DevExpress.XtraBars.ItemClickEventArgs) Handles btChaoGia.ItemClick
        gdvYCCT.CloseEditor()
            ...
            If gdvChuyenMaCT.GetRowCellValue(i, "SLYC") > 0 Then
                ...
                If Not tb Is Nothing Then
                    ...
                    If Convert.ToBoolean(chkThayThe.EditValue) Then
                        If Not ShowCauHoi("Thay thế mã: " & gdvCT.GetFocusedRowCellValue("Model") & " bằng bộ vật tư: " & gdvChuyenMaCT.GetRowCellValue(i, "Model")) Then Exit Sub
                    Else
                        For j As Integer = 0 To tb.Rows.Count - 1
                            ...
                            gdvYCCT.SetFocusedRowCellValue("IDVattu", tb.Rows(j)("ID"))
                            gdvYCCT.SetFocusedRowCellValue("TenVT", tb.Rows(j)("TenVT"))
                            gdvYCCT.SetFocusedRowCellValue("Thongso", tb.Rows(j)("Thongso"))
                            gdvYCCT.SetFocusedRowCellValue("Model", tb.Rows(j)("Model"))
                            gdvYCCT.SetFocusedRowCellValue("Soluong", tb.Rows(j)("SLTrongBo") * _SL)
                            gdvYCCT.SetFocusedRowCellValue("IDDVT", tb.Rows(j)("IDDVT"))
                            gdvYCCT.SetFocusedRowCellValue("TenHang", tb.Rows(j)("HangSX"))
                            gdvYCCT.SetFocusedRowCellValue("HangTon", tb.Rows(j)("HangTon"))
                            gdvYCCT.SetFocusedRowCellValue("slTon", tb.Rows(j)("slTon"))
                            gdvYCCT.SetFocusedRowCellValue("IDBo", tb.Rows(j)("IDBo"))
                            gdvYCCT.SetFocusedRowCellValue("SLBo", tb.Rows(j)("SLTrongBo"))
...
#Region "Các sự kiện liên quan đến grid yêu cầu gdvYCCT"
    ...
    Private Sub gdvYCCT_KeyDown(sender As System.Object, e As System.Windows.Forms.KeyEventArgs) Handles gdvYCCT.KeyDown
        ...
        If e.Control AndAlso (e.KeyCode = Keys.N Or e.KeyCode = Keys.G) Then
...
#Region "Lưu lại"
    ...
    Private Sub gdvYCCT_RowUpdated(sender As System.Object, e As DevExpress.XtraGrid.Views.Base.RowObjectEventArgs) Handles gdvYCCT.RowUpdated
        ...
        AddParameter("@IDBo", gdvYCCT.GetFocusedRowCellValue("IDBo"))
        AddParameter("@SLBo", gdvYCCT.GetFocusedRowCellValue("SLBo"))
        AddParameterWhere("@IDYC", gdvYCCT.GetFocusedRowCellValue("ID"))
...
#End Region
    ...
    Private Sub btMauFileYC_Click(sender As System.Object, e As System.EventArgs) Handles btMauFileYC.Click
        OpenFileOnLocal(ServerName & "\excel$\BIEUMAU\Mau File YC KH.xls", "Mau File YC KH.xls", True)
    End Sub
    ...
    Private Sub mPhanHoiChoCungUng_ItemClick(sender As System.Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles mPhanHoiChoCungUng.ItemClick
        ...
        Else
            If ShowCauHoi("Gửi thông báo lại cho nhân viên báo giá yêu cầu này ?") Then
                ...
                gdvYCCT.UpdateCurrentRow()
                'AddParameter("@TrangThai", Convert.ToInt32(TaiKhoan))
                'AddParameterWhere("@ID", gdvYCCT.GetFocusedRowCellValue("ID"))
                'If doUpdate("YEUCAUDEN", "ID=@ID") Is Nothing Then
                '    ShowBaoLoi(LoiNgoaiLe)
                'Else
    ...
    Private Sub cbTrangThai_EditValueChanged(sender As System.Object, e As System.EventArgs) Handles cbTrangThai.EditValueChanged
        If CType(sender, LookUpEdit).EditValue = TrangThaiYeuCau.CanHoiGiaHang Then
            Dim tg As DateTime = GetServerTime()
            ...
            If doUpdate("YEUCAUDEN", "ID=@IDYC") Is Nothing Then
                ShowBaoLoi(LoiNgoaiLe)
            Else
                ShowAlert("Đã cập nhật giờ yêu cầu !")
            End If
        End If
    End Sub

    'Private Sub gdvYCCT_LostFocus(sender As System.Object, e As System.EventArgs) Handles gdvYCCT.LostFocus
    '    gdvYCCT.CloseEditor()
    '    gdvYCCT.UpdateCurrentRow()
    'End Sub
    ...
    Private Sub gdvChuyenMaCT_MouseDown(sender As System.Object, e As System.Windows.Forms.MouseEventArgs) Handles gdvChuyenMaCT.MouseDown
        ...
        If e.Button = System.Windows.Forms.MouseButtons.Right Then
            pMenuChuyenMa.ShowPopup(gdvChuyenMa.PointToScreen(e.Location))
        End If
    End Sub

    Private Sub cbNVKD_EditValueChanged(sender As Object, e As EventArgs) Handles cbNVKD.EditValueChanged
        loadDSKH()
    End Sub
End Class
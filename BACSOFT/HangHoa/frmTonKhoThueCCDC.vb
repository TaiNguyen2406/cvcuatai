...
Public Class frmTonKhoThueCCDC


    Private Sub frmTonKhoThueCCDC_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
    ...
    Public Sub LoadDsPhongBan()
        ...
        If Not tb Is Nothing Then
            For Each r As DataRow In tb.Rows
                rCmbPhongBan.Items.Add(New ObjectItemCmb(r("Id"), r("PhongBan")))
            Next
        End If
    End Sub
    ...
    Private Sub LoadDuLieu()
        ...
        sql &= "SELECT ID, IdVatTu, SoLuong,SoKyPhanBo,SoKyConLai,GiaTriConLai,GhiChu,SoKyPhanBoTrongNam, "
        sql &= "(SELECT TenHoaDon FROM VATTU WHERE ID=tonkhothueccdc.IdVatTu )TenHoaDon, "
        sql &= "(SELECT MaPhongBan + ' - ' + TenPhongBan FROM PhongBanThue WHERE Id = tonkhothueccdc.IdPhongBan)PhongBan, "
        sql &= "(SELECT Ten FROM TENDONVITINH WHERE ID = (SELECT IDDonvitinh FROM VATTU WHERE ID=tonkhothueccdc.IdVatTu))DVT "
        sql &= "FROM tonkhothueccdc "
        If Not cmbPhongBan.EditValue Is DBNull.Value Then
            sql &= " WHERE IdPhongBan = " & CType(cmbPhongBan.EditValue, ObjectItemCmb).GiaTri & " "
        End If
    ...
    Private Sub mnuThemMoiCT_ItemClick(sender As System.Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles mnuThemMoiCT.ItemClick
        ...
        Dim f As New frmUpdateTonKhoThueCCDC
        f.Text = "Thêm mới tồn kho thuế CCDC"
    ...
    Private Sub mnuCapNhatCT_ItemClick(sender As System.Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles mnuCapNhatCT.ItemClick
        ...
        f.Text = "Cập nhật tồn đầu kỳ kho thuế CCDC"
        f.ShowDialog()
        LoadDuLieu()
    ...
    Private Sub mnuXoaBoCT_ItemClick(sender As System.Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles mnuXoaBoCT.ItemClick
        ...
        Try
            BeginTransaction()
            'Xóa chứng từ
            Dim sql As String = "DELETE FROM TONKHOTHUECCDC WHERE ID = " & gdvDataVT.GetFocusedRowCellValue("ID")

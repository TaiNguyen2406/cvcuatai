...
Public Class frmThue_BangNhapXuatTon
    ...
    Private Sub btnLocVT_ItemClick(sender As System.Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles btnLocVT.ItemClick
        ...
        sql = sql.Replace("{[TU_NGAY]}", Convert.ToDateTime(txtTuNgay.EditValue).ToString("dd/MM/yyyy"))
        sql = sql.Replace("{[DEN_NGAY]}", Convert.ToDateTime(txtDenNgay.EditValue).ToString("dd/MM/yyyy"))
        ...
        If chkCoNhapXuat.Checked Then
            strLoc2 &= " AND ID IN (SELECT DISTINCT IdVatTu FROM CHUNGTU LEFT OUTER JOIN CHUNGTUCHITIET ON CHUNGTU.Id = CHUNGTUCHITIET.Id_CT  "
            strLoc2 &= " WHERE Convert(datetime,Convert(nvarchar,CHUNGTU.NgayHD,103),103) >= @TuNgay "
            strLoc2 &= " AND Convert(datetime,Convert(nvarchar,CHUNGTU.NgayHD,103),103) <= @DenNgay )"
        End If

        sql = sql.Replace("{[DK_LOC_2]}", strLoc2)

        Dim strLoc1 As String = ""
        If chkVatTuTon.Checked Then
            strLoc1 &= " AND (SlTonDauKy+SlNhapTrongKy-SlXuatTrongKy) <> 0 "
        End If
        sql = sql.Replace("{[DK_LOC_1]}", strLoc1)

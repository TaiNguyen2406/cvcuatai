...
Public Class frmTHCongNo
    ...
    Private Sub frmTHCongNo_Load(sender As Object, e As System.EventArgs) Handles Me.Load
        ...
        LoadTuDien()
        If Not KiemTraQuyenSuDungKhongCanhBao("Menu", Me.Parent.Tag, DanhMucQuyen.Admin) And Not KiemTraQuyenSuDungKhongCanhBao("Menu", Me.Parent.Tag, DanhMucQuyen.KeToan) And Not KiemTraQuyenSuDungKhongCanhBao("Menu", Me.Parent.Tag, DanhMucQuyen.TPQuanLy) Then
            tabPhaiThu.PageVisible = False
            tabPhaiTra.PageVisible = False
            ' btfilterMaKH.Enabled = False
        End If
...
#Region "Load cb Data"
    ...
    Public Sub LoadTuDien()
        ...
        sql &= " SELECT ID,ttcMa FROM KHACHHANG "
        If Not KiemTraQuyenSuDungKhongCanhBao("Menu", Me.Parent.Tag, DanhMucQuyen.Admin) And Not KiemTraQuyenSuDungKhongCanhBao("Menu", Me.Parent.Tag, DanhMucQuyen.KeToan) And Not KiemTraQuyenSuDungKhongCanhBao("Menu", Me.Parent.Tag, DanhMucQuyen.TPQuanLy) Then
            sql &= " WHERE IDTakeCare=@NV"
            AddParameterWhere("@NV", TaiKhoan)
        End If
        sql &= " ORDER BY ttcMa"
...
#End Region
    ...
    Private Sub LoadPhaiThu()
        ...
        sql &= " LEFT JOIN (select isnull(sum((tientruocthue + tienthue)*tygia),0)PhaiThu,IDKhachHang from PHIEUXUATKHO where Convert(datetime,convert(nvarchar, ngaythang,103),103) between @NgayChotNoDauKy and @NgayChotCuoiKyXem GROUP BY IDKhachHang)tbPhaiThuDK ON tbPhaiThuDK.IDKhachHang=KHACHHANG.ID"
        ...
        sql &= " LEFT JOIN (select isnull(sum((tientruocthue + tienthue)*tygia),0)PhaiThu,IDKhachHang from PHIEUXUATKHO where Convert(datetime,convert(nvarchar, ngaythang,103),103) between @TuNgay and @DenNgay GROUP BY IDKhachHang)tbPhaiThuTK ON tbPhaiThuTK.IDKhachHang=KHACHHANG.ID"
        ...
        sql &= " LEFT JOIN (select isnull(sum(sotien),0)SoTien,IDKh from UNC where Mucdich = 230 and Ngaythang between @TuNgay and @DenNgay GROUP BY IDKh)tbUNCHUTK ON tbUNCHUTK.IDKh=KHACHHANG.ID "
        If Not btfilterMaKH.EditValue Is Nothing Then
            sql &= " WHERE KHACHHANG.ID=" & btfilterMaKH.EditValue
        End If
        sql &= ")tb"
    ...
    Private Sub LoadPhaiTra()
        ...
        'Phải trả đầu kỳ"
        sql &= " LEFT JOIN (select isnull(sum((tientruocthue + tienthue)*tygia),0)PhaiTra,IDKhachHang from PHIEUNHAPKHO where  Convert(datetime,convert(nvarchar, ngaythang,103),103) between @NgayChotNoDauKy and @NgayChotCuoiKyXem GROUP BY IDKhachHang)tbPhaiTraDK ON tbPhaiTraDK.IDKhachHang=KHACHHANG.ID"
        ...
        'Phải trả TK"
        sql &= " LEFT JOIN (select isnull(sum((tientruocthue + tienthue)*tygia),0)PhaiTra,IDKhachHang from PHIEUNHAPKHO where Convert(datetime,convert(nvarchar, ngaythang,103),103) between @TuNgay and @DenNgay GROUP BY IDKhachHang)tbPhaiTraTK ON tbPhaiTraTK.IDKhachHang=KHACHHANG.ID"
    ...
    Private Sub LoadChiTietPhaiThu()
        ...
        sql &= " , DonGia = Dongia * tygia, TienGang = (Soluong * Dongia * Tygia),   TienThue = (Soluong * Dongia * Tygia* Mucthue /100 * Xuatthue)"
            ...
            For i As Integer = 1 To tb.Rows.Count - 1
                If tb.Rows(i)("SP").ToString.Substring(0, 2) = "XK" Then
                    If tb.Rows(i)("SP").ToString <> tb.Rows(i - 1)("SP").ToString Then
                        ...
                        tb2.Rows.Add(r2)
                        If Not IsDBNull(tb.Rows(i)("PSNo")) Then ThuPSNo += tb.Rows(i)("TongTien")
            ...
            For i As Integer = 1 To tb.Rows.Count - 1
                If tb.Rows(i)("SP").ToString.Substring(0, 2) = "NK" Then
                    If tb.Rows(i)("SP").ToString <> tb.Rows(i - 1)("SP").ToString Then
                        ...
                        r2("IDs") = tb.Rows(i)("IDs")
                        tb2.Rows.Add(r2)
                        If Not IsDBNull(tb.Rows(i)("PSCo")) Then ChiPSCo += tb.Rows(i)("TongTien")
    ...
    Private Sub btTaiLai_ItemClick(sender As System.Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles btTaiLai.ItemClick
        ...
        ElseIf tabCongNo.SelectedTabPage Is tabChiTietPhaiThu Then
            If btfilterMaKH.EditValue Is Nothing Then
                ShowCanhBao("Chưa chọn mã khách hàng")
                Exit Sub
            End If
            LoadChiTietPhaiThu()
        Else
    ...
    Private Sub btKetXuat_ItemClick(sender As System.Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles btKetXuat.ItemClick
        ...
        Select Case tabCongNo.SelectedTabPage.Name
            ...
            Case tabChiTietPhaiThu.Name
                saveFile.FileName = "Chi tiet no phai thu " & Convert.ToDateTime(btfilterTuNgay.EditValue).ToString("dd-MM-yy") & "  " & Convert.ToDateTime(btfilterDenNgay.EditValue).ToString("dd-MM-yy") & ".xls"

            Case Else
                saveFile.FileName = "Chi tiet no phai tra " & Convert.ToDateTime(btfilterTuNgay.EditValue).ToString("dd-MM-yy") & "  " & Convert.ToDateTime(btfilterDenNgay.EditValue).ToString("dd-MM-yy") & ".xls"
        ...
        If saveFile.ShowDialog = DialogResult.OK Then
            ...
            Try
                ...
                ElseIf tabCongNo.SelectedTabPage Is tabPhaiTra Then
                    Utils.ExportGdv.ExportToExcel(saveFile.FileName, gdvPhaiTraCT, False)
                ElseIf tabCongNo.SelectedTabPage Is tabChiTietPhaiThu Then
                    Utils.ExportGdv.ExportToExcel(saveFile.FileName, gdvCTNoPhaiThuCT, False)
                Else
                    Utils.ExportGdv.ExportToExcel(saveFile.FileName, gdvCTNoPhaiTraCT, False)
                End If
    ...
    Private Sub btfilterTuNgay_EditValueChanged(sender As System.Object, e As System.EventArgs) Handles btfilterTuNgay.EditValueChanged
        btNgayChotCuoiKyXem.EditValue = DateAdd(DateInterval.Day, -1, Convert.ToDateTime(btfilterTuNgay.EditValue))

...
Public Class frmUpdateYcXuatTam
    ...
    Private Sub frmUpdateYcXuatTam_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        ...
        If TrangThai.isUpdate Then
            Try
                Dim sql As String = "select SoYC,SoCG,ThoiGianLap,ThoiGianCan,GhiChu,IdNguoiLap, "
                sql &= "(select ten from nhansu where id = phieuxuatkhotam.idnguoilap)TenNguoiLap "
                ...
                txtThoiGianCan.EditValue = dt.Rows(0)("ThoiGianCan")
    ...
    Private Sub LoadDsVatTu()
        ...
        sql &= " TENDONVITINH.Ten AS TenDVT,Soluong AS SoLuong,"
        ...
        sql &= " (SELECT isnull(SUM(SlYeuCau),0) FROM XUATKHOTAM WHERE IdVatTu = CHAOGIA.IDvattu AND SoCG = @SP)SlDaYeuCau,  "

        sql &= " convert(float,0)SlCon, "
        ...
        Else
            ...
            dt.Columns.Add(New DataColumn("IdXuatKhoTam", Type.GetType("System.Int64")))

            If TrangThai.isUpdate Then
                sql = "select id,SoCG,IdVatTu,SlYeuCau from xuatkhotam where id_phieu = @Id"
                AddParameter("@Id", idPhieu)
                ...
                For i As Integer = 0 To dt.Rows.Count - 1
                    ...
                    If arrRow.Length > 0 Then
                        dt.Rows(i)("IdXuatKhoTam") = arrRow(0)("Id")
                        dt.Rows(i)("SlYeuCau") = arrRow(0)("SlYeuCau")
                    End If
                Next
                Dim rDelete = dt.Select("IdXuatKhoTam is null")
                While rDelete.Length > 0
                    dt.Rows.Remove(rDelete(0))
                    rDelete = dt.Select("IdXuatKhoTam is null")
                End While
            End If
    ...
    Private Sub btnCapNhat_ItemClick(sender As System.Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles btnCapNhat.ItemClick
        ...
        Try
            ...
            AddParameter("@ThoiGianCan", txtThoiGianCan.EditValue)
            ...
            If TrangThai.isAddNew Or TrangThai.isCopy Then
                idPhieu = doInsert("PHIEUXUATKHOTAM")
            ...
            For i As Integer = 0 To gdvVTCT.RowCount - 1
                If gdvVTCT.GetRowCellValue(i, "SlYeuCau") = 0 Then
                    If gdvVTCT.GetRowCellValue(i, "IdXuatKhoTam") Is DBNull.Value Then
                        Continue For
                    Else
                        AddParameterWhere("@Id", gdvVTCT.GetRowCellValue(i, "IdXuatKhoTam"))
                        If doDelete("XUATKHOTAM", "Id=@Id") Is Nothing Then Throw New Exception(LoiNgoaiLe)
                    End If
                Else
                    ...
                    If gdvVTCT.GetRowCellValue(i, "IdXuatKhoTam") Is DBNull.Value Then
                        Dim idXKTam As Object = doInsert("XUATKHOTAM")
                        If idXKTam Is Nothing Then Throw New Exception(LoiNgoaiLe)
                        gdvVTCT.SetRowCellValue(i, "IdXuatKhoTam", idXKTam)
                    Else
                        AddParameterWhere("@Id", gdvVTCT.GetRowCellValue(i, "IdXuatKhoTam"))
    ...
    Public Shared Sub InPhieuXuaTam(id As Object)
        Try
            ...
            Dim f As New frmIn("In phiếu yêu cầu xuất kho tạm")
            Dim rpt As New rptPhieuXuatKhoTam
            rpt.pLogo.Image = My.Resources.Logo3


            Dim sql As String = ""
            sql &= "select SoYC as SoPhieu,SoCG,ThoiGianCan as NgayXuat,GhiChu, "
            ...
            rpt.lbTieuDe.Text = "PHIẾU YÊU CẦU XUẤT KHO TẠM"

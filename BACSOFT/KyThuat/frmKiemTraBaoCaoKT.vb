...
Public Class frmKiemTraBaoCaoKT
    ...
    Public Sub TheoXuatKho()
        ...
        'Tính lợi nhuận cho các xuất kho trong tháng
        ...
        sql &= "    * ISNULL(PHIEUXUATKHO.Tygia, 1) / (1 - (CASE WHEN BANGCHAOGIA.CongTrinh=0 THEN ISNULL(dbo.tblQuyDoi.HSThuCK, 0.15) ELSE (CASE WHEN BANGCHAOGIA.KhauTru is null THEN 0.15 ELSE BANGCHAOGIA.KhauTru/100 END) END))) * (ISNULL(tblTuDien.Diem, 0) / 100) END) AS LoiNhuanKT,"
        sql &= "    tblTuDien.HeSo1 as PTLoiNhuanThietKe"
        sql &= " INTO #tbLoiNhuanKT "
        ...
        sql &= " LEFT OUTER JOIN tblTuDien ON BANGYEUCAU.IDLoaiYeuCau = tblTuDien.ID "
        sql &= " LEFT OUTER JOIN"
        sql &= "        (SELECT     Sophieu, SUM(ISNULL(gianhap, giaban) * Soluong) AS GiaNhap"
        sql &= "          FROM          V_XuatkhoCal"
        ...
        sql &= " INTO #tbKetQua"
        sql &= " FROM("
        sql &= " SELECT #TongHop.*,tb.TongSoGio,h"
        sql &= " (CASE WHEN #tbMauSo.MauSo = 0 THEN 0 ELSE"
        sql &= " ((#TongHop.SoGio*#TongHop.TongDiemKN) /#tbMauSo.MauSo) * #TongHop.LoiNhuanNhomCV END)  AS SoTien"
        sql &= " FROM #TongHop"
        sql &= " INNER JOIN "

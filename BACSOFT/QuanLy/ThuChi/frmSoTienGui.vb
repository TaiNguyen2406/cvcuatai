...
Public Class frmSoTienGui
    ...
    Public Sub LoadSoTienGui()
        ...
        If Not cbSoTK.EditValue Is Nothing Then
            sql &= " SELECT (ISNULL((SELECT SUM(SoTien*isnull(TyGia,1)) FROM THUNH WHERE THUNH.NgayThangCT<@TuNgay "
            sql &= " AND rtrim(ltrim(TaiKhoanDen))='" & cbSoTK.EditValue & "'"
            sql &= " ),0) "
            sql &= " + ISNULL((SELECT SUM(SoTien*isnull(TyGia,1)) FROM CHI WHERE CHI.NgayThangCT<@TuNgay AND MucDich=211"
            sql &= " AND rtrim(ltrim(MaTK))='" & cbSoTK.EditValue & "'"
            sql &= " ),0)"
            sql &= " 		- ISNULL((SELECT SUM(SoTien*isnull(TyGia,1)) FROM UNC WHERE UNC.NgayThang<@TuNgay"
            sql &= " AND rtrim(ltrim(TaiKhoanDi))='" & cbSoTK.EditValue & "'"
            sql &= " ),0)"
            sql &= " 		- ISNULL((SELECT SUM(SoTien*isnull(TyGia,1)) FROM THU WHERE THU.NgayThangCT<@TuNgay AND MucDich=101"
            sql &= " AND rtrim(ltrim(MaTK))='" & cbSoTK.EditValue & "'"
            sql &= " ),0)"
            sql &= " )AS DauKy"
        Else
            sql &= " SELECT (ISNULL((SELECT SUM(SoTien*isnull(isnull(TyGia,1),1)) FROM THUNH WHERE THUNH.NgayThangCT<@TuNgay),0) "
            sql &= " 		+ ISNULL((SELECT SUM(SoTien*isnull(TyGia,1)) FROM CHI WHERE CHI.NgayThangCT<@TuNgay AND MucDich=211),0)"
            sql &= " 		- ISNULL((SELECT SUM(SoTien*isnull(TyGia,1)) FROM UNC WHERE UNC.NgayThang<@TuNgay),0)"
            sql &= " 		- ISNULL((SELECT SUM(SoTien*isnull(TyGia,1)) FROM THU WHERE THU.NgayThangCT<@TuNgay AND MucDich=101),0)"
            sql &= " )AS DauKy"
        End If
        ...
        sql &= " 	THUNH.SoTien*isnull(TyGia,1) SoTien,TienTe,MucDich,Convert(bit,0) AS THUCHI,TaiKhoanDen AS TaiKhoan"
        sql &= " FROM THUNH"
        sql &= " UNION ALL "
        ...
        sql &= " 	UNC.SoTien*isnull(TyGia,1) SoTien,TienTe,MucDich,Convert(bit,1) AS THUCHI,TaiKhoanDi AS TaiKhoan"
        sql &= " FROM UNC"
        sql &= " UNION ALL"
        sql &= " SELECT 0 AS STT, THU.ID,NgayThangVS,(N'RT ' + THU.SoPhieu) AS SoPhieu,NgayThangCT,THU.IDKh,THU.DienGiai,"
        sql &= " 	THU.SoTien*isnull(TyGia,1) SoTien,TienTe,MucDich,Convert(bit,1) AS THUCHI,MaTK AS TaiKhoan"
        sql &= " FROM THU"
        ...
        sql &= " 	CHI.SoTien*isnull(TyGia,1) SoTien,TienTe,MucDich,Convert(bit,0) AS THUCHI,MaTK AS TaiKhoan"
        sql &= " FROM CHI"
